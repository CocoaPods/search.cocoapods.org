module Pod
  ...
  class View < Struct.new(:id, :platforms, :version, :summary, :authors, :link, :subspecs)
    ...
    def render
      ...
      %Q{<li class="result">#{rendered_platform}<h3><a href="#{link}">#{id}</a>#{version}</h3><span class="pod_spec">#{pod_spec}</span> <span class="clippy">#{pod_spec}</span><p class="subspecs">#{rendered_subspecs}</p><p>#{summary}</p><p class="author">#{rendered_authors}</p></li>}
    end
