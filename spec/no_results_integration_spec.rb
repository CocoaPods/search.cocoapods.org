...
describe 'Integration Tests' do
  ...
  it 'will return the right tag facets' do
    Yajl.load(no_results.send_search)['tag'].keys.sort.should == ["alert", "api", "button", "client", "communication", "controller", "gesture", "http", "image", "json", "layout", "logging", "navigation", "network", "notification", "parser", "progress", "rest", "table", "test", "text", "view", "xml"]
  end
  it 'will return a correctly split query' do
    Yajl.load(no_results.send_search(query: 'meow'))['split'].should == [[], 0]
  end
  it 'will return a correctly split query' do
    Yajl.load(no_results.send_search(query: 'afnetworking'))['split'].should == [["networking"], 7]
  end
