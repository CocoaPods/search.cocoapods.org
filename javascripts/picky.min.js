...
"en",i=a.split(".").concat(b),f=dictionary,e=0,n=i.length;e<n;e++){f=f[i[e]];if(f==undefined){f="Translation missing: "+a+"."+b;break}}return f};function Allocation(a,b,i,f,e,n){var p=this;this.type=a;this.weight=b;this.count=i;this.combination=f;this.ids=e||[];this.entries=this.rendered=n;this.isType=function(s){return s==p.type}}function Allocations(a){this.allocations=[];for(var b=0,i=a.length;b<i;b++){var f=a[b];this.allocations.push(new Allocation(f[0],f[1],f[2],f[3],f[4],f[5]))}this.length=this.allocations.length;this.remove=function(e){this.allocations.splice(e,1)};this.each=function(e){this.allocations.each(e)}}
function PickyData(a){var b=a.total,i=a.duration,f=a.offset,e=new Allocations(a.allocations||[]);this.original_hash=a;this.total=b;this.duration=i;this.offset=f;this.allocations=e;this.renderedAmount=function(){var n=0;e.each(function(p,s){n+=s.rendered.length});return n};this.isEmpty=function(){return b==0}};var PickyView=function(a,b){var i=b.showResultsLimit||10,f=b.input,e=b.reset,n=b.button,p=b.counter,s=b.dashboard,u=b.moreSelector,x=b.results,y=b.noResults,j=new PickyAddination(this,x),r=new PickyAllocationsCloud(this,b),d=new PickyResultsRenderer(j,b),h=function(){e.fadeTo(166,1)},g=function(){r.hide();x.empty();y.hide()},q=function(k){f.val(k);e.fadeTo(166,0);v("empty");p.empty();g()},m=function(){return f.val()};this.text=m;var c=function(k){p.text(k);k>0&&k<=5&&p.fadeTo("fast",0.5).fadeTo("fast",
1)},o=function(k){if(k.isEmpty())return"none";if(k.total>i&&k.allocations.length>1)return"support";return"ok"},v=function(k){s.attr("class","dashboard "+k)};this.insert=function(k){f.val(k);f.select()};this.fullResultsCallback=function(k){v(o(k));if(k.isEmpty()){g();c(0);y.show();h()}else if(k.total>i&&k.allocations.length>1){g();h();r.show(k);c(k.total)}else if(k.offset==0){g();c(k.total);d.render(k);x.show();h();f.focus()}else{var w=$(u).position().top;j.remove();d.render(k);$("body").animate({scrollTop:w-
12},500)}};this.liveResultsCallback=function(k){v(o(k));c(k.total)};this.allocationChosen=function(k){k=k.data.query;f.val(k);a.allocationChosen(k)};this.addinationClicked=function(k){a.addinationClicked(m(),k)};(function(){f.keyup(function(k){if(m()==""){q();a.searchTextCleared()}else{a.searchTextEntered(m(),k);h()}});p.click(function(){m()==""||a.searchButtonClicked(m())});n.click(function(){m()==""||a.searchButtonClicked(m())});e.click(function(){q("");a.clearButtonClicked();f.focus()})})();f.focus()};var PickyBackend=function(a){var b=function(i,f,e){var n=e||{};n=$.extend({query:i},e);$.getJSON(a,n,function(p){f&&f(new PickyData(p))})};this.search=function(i,f,e,n){b(i,function(p){f&&f(n,p)},e)}},LiveBackend=function(a){var b=a.live||alert("A live backend path must be provided."),i=new PickyBackend(b);this.search=function(f,e,n,p){p=p||{};latestRequestTimestamp=new Date;p.live=latestRequestTimestamp;n=$.extend({ids:a.liveResults||0,offset:0},n);i.search(f,function(s,u){if(!s.live||s.live==latestRequestTimestamp)e&&
e(u)},n,p)}},FullBackend=function(a){var b=a.full||alert("A full backend path must be provided."),i=new PickyBackend(b);this.search=function(f,e,n,p){p=p||{};latestRequestTimestamp=new Date;p.full=latestRequestTimestamp;n=$.extend({ids:a.fullResults||20,offset:0},n);i.search(f,function(s,u){if(!s.full||s.full==latestRequestTimestamp)e&&e(u)},n,p)}};var PickyController=function(a){var b=new PickyView(this,a),i=a.backends,f=a.before||function(){},e=a.success||function(){},n=a.after||function(){},p=a.liveRendered||false,s=a.liveSearchInterval||180,u,x=function(c){return(c=c&&c.match(/q=([^&]+)/))&&decodeURIComponent(c[1]).replace(/\+/g," ").replace(/#/g,"")||""};this.extractQuery=x;var y=function(){var c=window.History&&window.History.getState();return x(c&&c.url)};this.lastFullQuery=y;var j=function(c,o,v,k){o=f(o,k)||o;u=[c,o,v,k];var w=o;if(w!=
y()){w="?q="+escape(w).replace(/\*/g,"%2A");window.History&&window.History.getState()&&window.History.pushState&&window.History.pushState(null,null,w)}(c=i[c])&&c.search(o,v,k)};this.resend=function(){u&&j.apply(this,u)};var r=function(c,o){c=e(c,o)||c;b.fullResultsCallback(c);n(c,o)},d=function(c,o){clearInterval(g);j("full",c,r,o||{})};a=function(c,o){c=e(c,o)||c;b.liveResultsCallback(c);n(c,o)};var h=p?r:a,g,q=function(){var c=b.text();j("live",c,h,{});clearInterval(g)};g=setInterval(q,s);clearInterval(g);
...
a.allocations.find(".more");a.hiddenAllocations=a.allocations.find(".hidden");a.maxSuggestions=a.maxSuggestions||3;a.results=$(a.resultsSelector||b+" div.results");a.resultsDivider=a.resultsDivider||"";a.noAsterisks=a.noAsterisks||[];a.wrapResults=a.wrapResults||'<ol class="results"></ol>';var i=a.controller&&new a.controller(a)||new PickyController(a);var f=this.insert=function(e,n,p){i.insert(e,n||{},p||true)};this.resend=i.resend;this.insertFromURL=function(e){if(e&&e!="")f(e);else(e=i.lastFullQuery())&&
f(e)}};var PickyAddination=function(a,b){this.remove=function(){b.find(".addination").remove()};this.render=function(i){var f=i.total,e,n=i.renderedAmount();e=i.offset+n;n=e+n;i=i.total;if(i<n)n=i;e={offset:e,start:e+1,end:n};if(e.offset<f){f=$("<div class='addination'>"+t("results.addination.more")+"</div>");f.bind("click",{offset:e.offset},a.addinationClicked);return f}else return""}};var PickyResultsRenderer=function(a,b){var i=b.results,f=b.resultsDivider,e=b.wrapResults,n=b.noAsterisks,p=function(j){var r=j[j.length-1];j=j.slice(0,j.length-1);if(j==[])j=[j];if(!n.include(r[0]))if(r[1].match(/[^\*~]$/))r[1]+="*";j.push(r);return j},s=function(j){for(var r=Localization.explanations&&Localization.explanations[PickyI18n.locale]||{},d=[],h,g=0,q=j.length;g<q;g++){h=j[g];var m=h[0];m=r[m]||m;d.push([m,h[1]])}return d},u=function(j,r){return[j.replace(/([\w\s\u00c4\u00e4\u00d6\u00f6\u00dc\u00fc\u00e9\u00e8\u00e0]+)/,
...
return d};this.render=function(j){j.allocations.each(function(r,d){if(d.entries.length>0){i.append(y(j,d)).append(d.entries.join(f));i.children("li").wrapAll(e)}});i.append(a.render(j))}};function AllocationRenderer(a,b){function i(d){var h={},g={},q=[],m;m=0;for(l=d.length;m<l;m++){var c=d[m][0];if(c in h){h[c]=h[c]+" "+d[m][1];q.push(m)}else{h[c]=d[m][1];g[m]=c}}for(m in g)d[m][1]=h[g[m]];for(m=q.length-1;m>=0;m--)d.remove(q[m]);return d}function f(d){return $.map(d,function(h,g){return"%"+(g+1)+"$s"}).join(" ")}function e(d){if(d.length==0)return"";var h=d=i(d);h.sort(function(w,z){return w[0]<z[0]?-1:1});for(var g=[],q=0,m=h.length;q<m;q++)g.push(h[q][0]);var c=g.length==1,o=j[g]||
(j[g]=f(g));if($.type(o)==="string"){j[g]={format:o};o=j[g]}var v=1,k=o.format;$.each(d,function(w,z){var A=z[0],B=z[2];if(o.filter)B=o.filter(B);A=x[A]||A;if(c&&!(o&&o.ignoreSingle))return k=B+"&nbsp;("+A+")";k=k.replace(RegExp("%"+v+"\\$s","g"),B);v+=1;return v});return k}function n(d){for(var h=[],g=0,q=y.length;g<q;g++)h.push([]);h.push([]);g=0;for(q=d.length;g<q;g++){for(var m=d[g],c=m[0],o=false,v=0,k=y.length;v<k;v++)if(y[v].include(c)){h[v].push(m);o=true;break}o||h[h.length-1].push(m)}var w;
for(d=h.length-1;d>=0;d--){w=h[d];if(w.length>0)break}w=w[w.length-1];r.include(w[0])||(w[1]+="...");return $.map(h,function(z){return e(z)})}function p(d){var h=[],g,q;for(q in d){g=d[q][0];g=u[g]||g;h[q]=g+":"+d[q][2]}return h.join(" ")}var s=PickyI18n.locale,u=Localization.qualifiers&&Localization.qualifiers[s]||{},x=Localization.explanations&&Localization.explanations[s]||{},y=b.groups||[],j=Localization.choices&&Localization.choices[s]||{};this.explanation=this.query=this.text="";var r=["street_number",
"zipcode"];this.contract=i;this.rendered=e;this.groupify=n;this.querify=p;this.render=function(d){var h=d.combination,g=d.count;d=p(h);h=n(h).join(" ");h=$('<li><div class="text">'+h+'</div><div class="count">'+g+"</div></li>");h.bind("click",{query:d},a);return h}};var PickyAllocationsCloud=function(a,b){var i=b.allocations,f=b.shownAllocations,e=b.showMoreAllocations,n=b.hiddenAllocations,p=b.maxSuggestions,s=function(){i.hide()},u=new AllocationRenderer(function(j){s();a.allocationChosen(j)},b),x=function(j){var r=[];j.each(function(d,h){r.push(u.render(h))});return r},y=function(j){if(j.length==0)return i.hide();f.empty();e.hide();n.empty().hide();if(j.length>p){$.each(j.slice(0,p-1),function(r,d){f.append(d)});$.each(j.slice(p-1),function(r,d){n.append(d)});
e.show()}else $.each(j,function(r,d){f.append(d)});return i.show()};e.click(function(){e.hide();n.show()});this.hide=s;this.show=function(j){y(x(j.allocations));i.show()}};
